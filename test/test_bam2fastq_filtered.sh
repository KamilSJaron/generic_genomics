...
echo "1_Tdi_b3v04_scaf000001" > list_to_test
...
if [[ $(zcat filt_one_R1.fastq.gz | wc -l) -eq 7 ]]; then
   echo "filter one works";
fi

if [[ $(zcat filt_both_R1.fastq.gz | wc -l) -eq 8 ]]; then
   echo "filter one works";
fi

if [[ $(keep_one_R1.fastq.gz | wc -l) -eq 3 ]]; then
   echo "filter one works";
fi

if [[ $(zcat keep_both_R1.fastq.gz | wc -l) -eq 2 ]]; then
   echo "filter one works";
fi
